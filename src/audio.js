...
function playStoryAlarm() {
  // ★ ここだけは ADV の音量規則から外れる。
  //   村がこれから焼かれる合図なので、遠慮すると場面が嘘になる。
  //   厚みの作り方は3つ:
  //     1. 同じ音を3オクターブ重ねる(622 / 311 / 155Hz)
  //     2. わずかにずらした音を足して唸らせる(624Hz。2Hzの差が「うねり」になる)
  //     3. 鳴り始めに金属質の打撃を差す(サイレンの筐体が叩かれる音)
  const N = 6;                    // 4回 → 6回。鳴り止まない感じにする
  for (let i = 0; i < N; i++) {
    const d = i * 0.60;
    const grow = 1 + i * 0.06;    // だんだん強くなる。近づいてくるように聞こえる

    // 高い方
    playTone(622, 0.32, 0.260 * grow, 'square',   d);
    playTone(624, 0.32, 0.150 * grow, 'sawtooth', d);      // 2Hzずらして唸らせる
    playTone(311, 0.32, 0.190 * grow, 'square',   d);
    playTone(155, 0.34, 0.150 * grow, 'square',   d);      // 腹に来る低音

    // 低い方
    playTone(466, 0.30, 0.240 * grow, 'square',   d + 0.30);
    playTone(468, 0.30, 0.140 * grow, 'sawtooth', d + 0.30);
    playTone(233, 0.30, 0.180 * grow, 'square',   d + 0.30);
    playTone(117, 0.32, 0.140 * grow, 'square',   d + 0.30);

    // サイレンの筐体を叩く音。1回ごとの頭に差す
    playNoise(0.05, 0.070 * grow, 4200, 1200, 'bandpass', d);
  }
  // 最後に長い余韻。切れたあとも耳に残す
...
// 貨物船が撃たれた。自機の被弾より鈍く、重く、遠い ―
// 「自分ではない大きなものが殴られている」音にする。
function playFreighterHit() {
  playNoise(0.34, 0.190, 900, 160, 'lowpass');       // 装甲が凹む
  playTone(74, 0.42, 0.150, 'square');               // 船体に響く低音
  playTone(112, 0.26, 0.090, 'square', 0.04);
}
...
function playRadioOpen() {
  // ★ 前の版は聞こえなかった。原因は音量と長さ ―
  //   最大 0.115(ロック音は 0.34)、いちばん長い音でも 55ms しかなく、
  //   エンジン音とBGMの下に完全に埋もれていた。
  //   他の効果音と同じ土俵(0.2〜0.3)まで上げ、全体で 0.23秒に伸ばす。
  //   6〜11秒に一度しか鳴らないので、これでうるさくはならない。

  // 1. 回線が開く「ザッ」。帯域を絞った雑音 ― 無線らしさはここで決まる
  playNoise(0.075, 0.30, 3200, 700, 'bandpass');

  // 2. 呼び出しの二音「ピッ・ポッ」。下がる形にすると呼びかけに聞こえる
  playTone(1180, 0.075, 0.24, 'square', 0.030);
  playTone( 880, 0.090, 0.20, 'square', 0.095);

  // 3. 低い胴鳴り。小さな音でも耳に残るのはこの成分で、
  //    これがないと甲高いだけで戦闘音に負ける
  playTone( 220, 0.090, 0.13, 'square', 0.030);

  // 4. 閉じるときの短い雑音。開いた音と対にすると「回線」だと分かる
...
const BGM = {
  // ★ 音量は実測で決めた。効果音の感覚で置くと必ず埋もれる。
  //   飛行中はエンジン音が鳴りっぱなしで、これが masterGain 通過後 -25.6 dB。
  //   BGM は低音が主体(この曲は120Hz以下が60%)で、エンジンと同じ帯域を奪い合う。
  //   最初 0.30 で入れたところエンジンより 8.1 dB 下になり、まったく聞こえなかった。
  //   1.00 でエンジンより +2.4 dB 上。ただし10分ずっと鳴り続けるので、
  //   それだと主張が強すぎた(「うっとおしい」)。0.72 でエンジンとほぼ同じ高さ。
  //   長い任務の下敷きとしては、これくらいで足りる。うるさければここを下げる。
  GAIN:      0.72,   // BGM の音量。効果音(AUDIO.MASTER)とは別に持つ
