...
const TEXT = {
  ...
  'wing.n2':            ['二番機', 'No.2'],
  'wing.n3':            ['三番機', 'No.3'],
  'brief.title':        ['アルカディア防衛戦', 'The Defence of Arcadia'],
  'brief.goal':         ['%s秒 持ちこたえる', 'Hold out for %s seconds'],
  'brief.goal2':        ['村を落とさせないためではない ― 逃がすための時間',
                         'Not to save the village — to buy time for those leaving it'],
  ...
  'def.down':           ['被撃墜', 'down'],
  // ★ 結果の見出しは勝敗で変えない。
  //   どちらでも村は落ちる ― それがこの章の内容そのものなので、
  //   「MISSION COMPLETE」のような達成の言葉を置いてはいけない。
  //   小説v2の一行をそのまま持ってくる。
  'def.title':          ['一時間もたなかった', 'THEY DID NOT LAST AN HOUR'],
  'def.held':           ['防衛隊 四機 ― 残存 %s / アルカディア 陥落',
                         'Four boats — %s left / Arcadia has fallen'],
  'def.lost':           ['防衛隊 四機 ― 全滅 / アルカディア 陥落',
                         'Four boats — all lost / Arcadia has fallen'],
  'def.burnResult':     ['果樹園 焼失', 'Orchard burned'],
  'def.heldTime':       ['持ちこたえた時間', 'Held for'],
...
// ★ 起動時にも一度あてる。
//   これまで applyLangToDom() は setLanguage()(言語を切り替えたとき)からしか
//   呼ばれていなかった。data-i18n の要素はHTMLに日本語が直接書いてあるので
//   気づかなかったが、data-i18n-html の3つは空の <div> のままだった ―
//   Tab の操作説明(1989文字)が丸ごと出ていなかったのはこれが原因。
//   lang.js はいちばん最初に読むが、<script> は body の末尾にあるので
//   ここへ来た時点で計器のHTMLはもう組み上がっている。
applyLangToDom();
