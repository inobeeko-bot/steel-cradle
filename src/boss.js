...
const BOSS = {
  ...
  // 体当たり判定の余裕。自機の当たり半径(COLLIDE.PLAYER_RADIUS = 3.0)に
  // これを足した距離まで近づくと即死になる。
  //
  // 以前は 2.2 で、合計 5.2。さらに当時は舷側の判定が最大1.53ぶん外へ
  // 張り出していたので、実際には艦体から 6.7 手前で死んでいた ―
  // 全長4の戦闘機にとって、機体1.7機ぶん手前。
  // 艦体を6段に分けて張り出しを 0.42 まで詰めたので、こちらも詰める。
  // 自機のいちばん外(翼端)は中心から2.3なので、3.0+0.6 でまだ余裕がある。
  RAM_MARGIN: 0.6,
...
function buildBossHull() {
  ...
  const rear  = bossCrossSection(1);   // 艦尾の断面。エンジンの取り付けに使う

  // ===================================================================
  // 艦体を、断面の輪(台形)を前後にいくつも並べて張る。
  //
  // 【なぜ艦首と艦尾の2枚だけで張らないのか】
  //
  // 以前は8頂点 ―艦首の台形と艦尾の台形だけ― で張っていた。
  // 断面の関数から頂点を取っているので当たり判定と一致するはずだったが、
  // 舷側で最大1.53ぶん、何も無い空間で弾が弾かれていた。
  //
  // 原因は三角形。舷側の面は四角形だが、3Dの四角形は必ず三角形2枚に割られる。
  // 艦首側は細く低いので4つの角が同一平面に乗らず、
  // 割られた2枚の平らな三角形は、当たり判定が見ている
  // 「前後にも上下にも滑らかに変化する面」の内側を通る。その差が張り出しになる。
  //
  // 輪を増やせば、1区画あたりのねじれが段数の2乗で小さくなる。
  //
  // 【なぜ12段なのか ― 艦首がいちばん厳しい】
  // 6段では舷側0.42・甲板0.22・船底0.00・艦尾0.00まで詰まったが、
  // 艦首から正面へ突っ込むと 1.33 も手前で死んでいた。
  //
  // これは艦首だけ別のバグ、ではない。面から見た張り出しはどこも0.37ほどで同じで、
  // 艦首は舷側の面が進行方向とほぼ平行(傾き16度)に寝ているため、
  // 同じ0.37が進む向きには 1/sin(16度) = 3.6倍に見えるだけ。
  // とはいえ正面から突っ込むのは戦い方として当たり前なので、そこを詰める。
  //
  // 12段にすると面からの張り出しが 0.37 → 0.09 になり、
  // 艦首方向でも 1.33 → 0.33 に収まる。三角形は100枚 ― 戦艦1隻ぶんなら安い。
  //
  // 見た目は変わらない。1区画ごとの面の傾きの差は0.1度ほどで、
  // 輪郭線(EdgesGeometry の既定はしきい値1度)には出ないので、
  // 単色フラット+輪郭線という見え方もそのまま保たれる。
  // ===================================================================
  // 【断面の輪に「中段」を足してある理由 ― 艦首の張り出しの正体】
  //
  // 前後を48段まで刻んでも、艦首の張り出しだけ 0.39 から下がらなかった。
  // 方向ごとに測り直すと、誤差は **y=0(舷側のちょうど中央の高さ)にだけ**
  // 集中していた:
  //
  //   高さ    x=4 での張り出し
  //   -----  ---------------
  ...
  //   y=  6       0.00
  //
  // 理由は、舷側の四角形が **高さ方向には1枚のまま**だったから。
  // 前後にいくら刻んでも、上下方向の分割は増えないので、
  // 平らな三角形2枚と「上下にも滑らかに変化する面」との差は縮まらない。
  // いちばん開くのは四角形の真ん中 = 中央の高さ、というわけ。
  //
  // そこで輪を4頂点から6頂点にして、舷側を上下2枚に割った。
  // 中段の半幅は上下の中点そのもの ―
  // 当たり判定(bossPointInsideWedge)が使う直線補間の値と一致するので、
  // 中段の線の上では誤差がぴったり0になる。
  //
  // そのうえで前後の段数を上げると、艦首の張り出しは 0.39 → 0.02 まで落ちた:
  //
  //   段数   艦首の張り出し
  //   ----  -------------
  //    24        0.21
  ...
  //    96        0.02(これ以上は下がらない)
  //
  // 艦首がとくに厳しいのは、舷側の面が進む向きとほぼ平行に寝ているため。
  // 面から見たわずかなずれが、進行方向には何倍にも見える
  // (甲板は同じ作りで 0.03、船底と艦尾は 0.00)。
  //
  // 三角形は 6面×2枚×48段 + ふた8枚 = 584枚。戦艦1隻ぶんなら安い
  // (背景のコロニーは2318枚と670枚)。
  const SEG = 48;
  const v = [];
  for (let i = 0; i <= SEG; i++) {
    ...
    const midW = (cs.halfW + topW) / 2;   // 中央の高さでの半幅(判定の式と同じ値)
    v.push(-cs.halfW, -cs.halfH, z);   // +0 左下
    v.push( cs.halfW, -cs.halfH, z);   // +1 右下
    v.push(-midW,     0,         z);   // +2 左中
    v.push( midW,     0,         z);   // +3 右中
    v.push(-topW,     cs.halfH,  z);   // +4 左上
    v.push( topW,     cs.halfH,  z);   // +5 右上
  }
  ...
  for (let i = 0; i < SEG; i++) {
    const a = i * 6;          // 手前の輪
    const b = (i + 1) * 6;    // ひとつ後ろの輪
    idx.push(a+0, b+1, b+0,  a+0, a+1, b+1);   // 下面
    idx.push(a+4, b+4, b+5,  a+4, b+5, a+5);   // 上面
    idx.push(a+0, b+0, b+2,  a+0, b+2, a+2);   // 左舷(下半分)
    idx.push(a+2, b+2, b+4,  a+2, b+4, a+4);   // 左舷(上半分)
    idx.push(a+1, a+3, b+3,  a+1, b+3, b+1);   // 右舷(下半分)
    idx.push(a+3, a+5, b+5,  a+3, b+5, b+3);   // 右舷(上半分)
  }
  const s = SEG * 6;
  // ふたは六角形なので三角形2枚では足りない。中段で割って4枚にする
  idx.push(0, 2, 3,  0, 3, 1,  2, 4, 5,  2, 5, 3);                       // 艦首
  idx.push(s+0, s+1, s+3,  s+0, s+3, s+2,
           s+2, s+3, s+5,  s+2, s+5, s+4);                               // 艦尾
...
// 【なぜ「面の平面」で判定するのか】
//
// 以前は断面の関数(bossCrossSection)から、その場所の半幅を
// 高さと前後位置の両方で線形に補間して出していた。
// 頂点は同じ関数から作っているので一致するはずだった ― が、ずれていた。
//
// 理由は三角形。舷側の面は四角形だが、3Dの四角形は必ず三角形2枚に割られる。
// 4つの角が同一平面上に無い(艦首側が細く低い楔なので、まさにそうなる)と、
// 割られた2枚の平らな三角形は、両方向に線形補間した「ふくらんだ曲面」の
// 内側を通る。その差が、何も無い空間に張り出した当たり判定になっていた。
//
// 実測(艦を真横から見て、判定の縁とポリゴンの縁の差):
//   艦首      0.00
//   t=0.3   ＋1.53   ← 面の中央がいちばんふくらむ
//   t=0.5   ＋1.38
//   艦尾      0.00
// 体当たりの余裕(RAM_MARGIN 2.2)を足すと、舷側では約3.7ぶん手前で
// 死ぬことになる。戦闘機の全長が4なので、機体1機ぶん近い。
//
// 【直し方:判定ではなくメッシュのほうを寄せた】
//
// はじめは「面の平面すべての内側か」で判定しようとした。艦体が凸なら厳密になる。
// だが艦体は厳密には凸ではない ― ねじれた四角形を三角形2枚に割ると、
// その折れ目のぶんだけわずかに凹む。平面で切ると実体まで削れてしまい、
// 実測で艦体の14.6%(舷側と下面)が「見えているのに当たらない」状態になった。
//
// そこで逆に、メッシュのほうを判定へ寄せた ―
// 艦体を前後6段に分けて張るようにした(buildBossHull)。
// 1区画あたりのねじれは段数の2乗で小さくなるので、
// 張り出しは 1.53 → 0.04 まで落ちる。判定の式はここに戻してある。
//
// ※ 開発者からの提案は「直方体の細かい集合にしては」だったが、
//   舷側は斜めなので、箱を並べると階段状になってむしろ緩くなる。
//   斜めの面は、斜めのまま式で持つのがいちばん正確で、しかも速い。
//
